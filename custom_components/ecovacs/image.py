...
from deebot_client.capabilities import CapabilityMap, DeviceType
...
async def async_setup_entry(
    ...
    for device in controller.devices:
        ...
        is_mower = device.capabilities.device_type is DeviceType.MOWER
        _LOGGER.warning(
            "image.py: device %s has map capability: %s",
            device.device_info.get("nick"),
            caps is not None,
        )
        if caps and device.map is not None:
            _LOGGER.warning("image.py: creating EcovacsMap entity")
            entities.append(EcovacsMap(device, caps, hass))
        elif is_mower:
            _LOGGER.warning(
                "image.py: creating GOAT fallback map entity without active map capability"
            )
            entities.append(EcovacsMap(device, caps, hass))
        elif caps:
...
class EcovacsMap(
    ...
    def __init__(
        ...
        self._map = cast(Map | None, self._device.map)
    ...
    def image(self) -> bytes | None:
        ...
        # The Rust map module returns None for mowers because it is designed for
        # vacuum pixel maps.
        if self._device.capabilities.device_type is DeviceType.MOWER:
            # Get zones from our _zone_store. map_data.map_subsets ignores ROOMS
            # type, so the GOAT renderer reads the explicit global stores.
            ...
            if self._map is not None:
                positions = getattr(
                    getattr(self._map, "_map_data", None), "_positions", []
                )
    ...
    def _placeholder_image(self) -> bytes:
        """Return a visible placeholder until map data arrives."""
        placeholder = (
            '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 400">'
            '<rect width="400" height="400" fill="#1a1a2e"/>'
            '<text x="200" y="180" font-family="sans-serif" font-size="16" '
            'fill="#4a9eff" text-anchor="middle">GOAT A3000 LiDAR</text>'
            '<text x="200" y="210" font-family="sans-serif" font-size="14" '
            'fill="#888" text-anchor="middle">Waiting for map data</text>'
            "</svg>"
        )
    ...
    async def async_added_to_hass(self) -> None:
        ...
        async def on_changed(event: MapChangedEvent) -> None:
            _LOGGER.warning(
                "MapChangedEvent received in image entity - clearing cache and updating"
            )
        ...
        if self._capability is not None:
            ...
            _LOGGER.warning("Image entity subscribed to map capability events successfully")
        elif self._device.capabilities.device_type is DeviceType.MOWER:
            self._subscribe(MapChangedEvent, on_changed)
            _LOGGER.warning(
                "Fallback mower image entity subscribed to MapChangedEvent successfully"
            )
